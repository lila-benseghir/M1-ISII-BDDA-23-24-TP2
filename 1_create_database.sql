...
create table typeprojet ( typeprj varchar(10) primary key not null check ( typeprj in ('PNR', 'CNEPRU' , 'AC' , 'CI' ))  , libellé varchar(60));
create table Chercheur (numch INT primary key not null check (numch <9 )  ,nomch varchar(30),genre varchar(2) check ( genre in ('M','F')) , codelab  varchar(4)  ,codeuniversité varchar(10)  );
